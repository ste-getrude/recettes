...
class Receipy(models.Model):
    ...
    receipy_yield = Model.SmallIntegerField() # pas claire, peut etre string
    
    
class RawIngredientLine(models.Model):
    receipy = models.ForeignKey(Receipy, on_delte=models.CASCADE)
...
class ReceipyIngredient(models.Model):
    receipy = models.ForeignKey(Receipy, on_delte=models.CASCADE)
    qty = models.FloatField(None=True)
